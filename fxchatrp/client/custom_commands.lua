...
RegisterCommand("maletero", function(source, args, raw)
    ...
    if IsPedInAnyVehicle(ped, false) then
        if GetVehicleDoorAngleRatio(veh, door) > 0 then
            ...
            FX.GtaNotification("[Vehículo] ~g~Maletero cerrado.")
        else
            SetVehicleDoorOpen(veh, door, false, false)
            FX.GtaNotification("[Vehículo] ~g~Maletero abierto.")
        end
    else
        if distanceToVeh < 6 then
            ...
            FX.GtaNotification("[Vehículo] ~y~Muy lejos del vehículo.")
        end
    end
end)
...
RegisterCommand("capo", function(source, args, raw)
    ...
    if IsPedInAnyVehicle(ped, false) then
        if GetVehicleDoorAngleRatio(veh, door) > 0 then
            SetVehicleDoorShut(veh, door, false)
            FX.GtaNotification("[Vehículo] ~g~Capó cerrado.")
        else
            SetVehicleDoorOpen(veh, door, false, false)
            FX.GtaNotification("[Vehículo] ~g~Capó abierto.")
        end
    else
...
RegisterCommand("puerta", function(source, args, raw)
    ...
    elseif args[1] == "6" then
        ...
        door = nil
        FX.GtaNotification("Uso: ~n~~b~/puerta [1,2,3,4]")
        FX.GtaNotification("~y~Puertas:")
        FX.GtaNotification("1(Delantera izquierda), 2(Delantera derecha)")
        FX.GtaNotification("3(Trasera izquierda), 4(Trasera derecha)")
    end
    ...
    if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		...
		if (GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1)) then
            ...
            if GetIsVehicleEngineRunning(veh) == false then
				...
				FX.GtaNotification("Motor ~g~encendido")
			else
                SetVehicleEngineOn(veh, false, true, true)
				SetVehicleJetEngineOn(veh, false)
				FX.GtaNotification("Motor ~r~apagado")
			end
		end
    end
end)
